...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default="Qwen3-VL-4B-Thinking",
                        help='model name.')
    parser.add_argument('--dataset', type=str, default='MATH500',
                        help='dataset name.')
    parser.add_argument('--generation_save_path', type=str, default="./Data/Eval/MATH500/Qwen3-VL-4B-Thinking/MATH500-Qwen3-VL-4B-Thinking_0.0_all_eval_vote_num1.json",
...
def sort_files_numerically(file_list):
    """Sort filenames by the first integer found in the name, fallback to lexicographic."""
    def num_key(f):
        m = re.search(r'(\d+)', f)
        return int(m.group(1)) if m else float('inf')
    return sorted(file_list, key=num_key)

# Replace the unsorted listing:
representation_files = sort_files_numerically(os.listdir(representation_path))
print("Total representation files: ", len(representation_files))
# ...existing code...
...
# %%
dups = df_accuracy[df_accuracy.duplicated("problem", keep=False)]
print(len(dups), dups["problem"].value_counts().head())
...
df_left = data_information_df.reset_index(drop=True).assign(row_id=lambda x: x.index)
df_right = df_accuracy.reset_index(drop=True).assign(row_id=lambda x: x.index)

data_information_df = df_left.merge(
    df_right[['row_id', 'accuracy']],
    on='row_id',
    how='left',
    validate='one_to_one',
).drop(columns='row_id')
...
# PCA + RidgeCV pipeline to reduce overfitting
n_comp = max(8, min(128, X_train.shape[0]-5, X_train.shape[1]))
model = Pipeline([
    ("scale", StandardScaler(with_mean=True, with_std=True)),
    ("pca", PCA(n_components=n_comp, random_state=42)),
    ("reg", RidgeCV(alphas=np.logspace(-4, 4, 50), cv=5)),
])
...
for idx, file in enumerate(representation_files):
    ...
    # Use PCA + RidgeCV pipeline to control capacity and generalize
    n_comp = max(8, min(128, X_train.shape[0]-5, X_train.shape[1]))
    model = Pipeline([
        ("scale", StandardScaler(with_mean=True, with_std=True)),
        ("pca", PCA(n_components=n_comp, random_state=42)),
    ...
    try:
        scale = model.named_steps.get('scale', None)
        pca = model.named_steps.get('pca', None)
        reg = model.named_steps.get('reg', None)

        coef_pca = getattr(reg, 'coef_', None)
        intercept_reg = getattr(reg, 'intercept_', None)

        coef = None
        intercept = None

        if coef_pca is not None:
            if coef_pca.ndim == 1:
                coef_orig = coef_pca
                if pca is not None:
                    coef_orig = pca.components_.T @ coef_orig
                if scale is not None:
                    coef_orig = coef_orig / scale.scale_
                coef = coef_orig
                intercept = float(intercept_reg)
                if scale is not None:
                    intercept = float(intercept_reg - np.dot(coef_orig, scale.mean_))
            else:
                coef_rows = coef_pca
                if pca is not None:
                    coef_rows = coef_rows @ pca.components_
                if scale is not None:
                    coef_rows = coef_rows / scale.scale_
                coef = coef_rows
                intercept = intercept_reg
                if scale is not None and intercept_reg is not None:
                    intercept = intercept_reg - coef_rows @ scale.mean_

    except Exception:
        coef = None
        intercept = None

    param_list['coef'].append(coef)
    param_list['intercept'].append(intercept)
...
# %%
loaded_data_temp = np.load("mean_steering_vectors_all-2.npy")

# %%
a = np.load("Assets/MathV/Qwen3-VL-8B-Thinking/mean_steering_vectors_all.npy")
a2 = np.load("Assets/MathVMini/Qwen3-VL-8B-Thinking/mean_steering_vectors_all.npy")

# %%
diff_dfs[20]['diff_embedding'].values


# %%

np.mean(diff_dfs[20]['diff_embedding'].values, axis=0)
...
# Save.
np.save(f'{base_asset_path}/mean_steering_vectors_all.npy', mean_steering_vectors[:-1])
#%%
...
# %%
import inspect
# 前提：representation_path 和 data_information_df 已经就绪
lengths = data_information_df["avg_tokens_reasoning"].to_numpy()
n = len(lengths)
assert n > 0, "no samples found"

low_thr = np.quantile(lengths, 0.3)
high_thr = np.quantile(lengths, 0.7)
idx_short = np.where(lengths <= low_thr)[0]
idx_long = np.where(lengths >= high_thr)[0]
print(f"short={len(idx_short)}, long={len(idx_long)}")

# 选层：-1 为最后一层；需要别的层改层号
repr_files = sorted([f for f in os.listdir(representation_path)
                     if f.startswith("embeds_") and f.endswith(".npy")],
                    key=lambda x: int(os.path.splitext(x)[0].split("_")[-1]))
layer_id = 30
target_file = repr_files[layer_id]
emb = np.load(os.path.join(representation_path, target_file))
assert emb.shape[0] == n, "embeddings/sample count mismatch"

emb_short = emb[idx_short]
emb_long = emb[idx_long]
mean_short = emb_short.mean(axis=0, keepdims=True)
mean_long = emb_long.mean(axis=0, keepdims=True)

X = np.vstack([emb_short, emb_long, mean_short, mean_long])
labels = (["short"] * len(idx_short) + ["long"] * len(idx_long)
          + ["Mean short", "Mean long"])
is_mean = [False] * (len(idx_short) + len(idx_long)) + [True, True]

# TSNE 兼容老版本：仅在支持时添加 n_iter
tsne_kwargs = dict(n_components=2, perplexity=30, learning_rate=200,
...
palette = {"short": "#1f77b4", "long": "#d62728",
           "Mean short": "#0c3c74", "Mean long": "#7f1d1d"}

# 普通点
sns.scatterplot(data=df_tsne[~df_tsne["is_mean"]],
                x="tsne_x", y="tsne_y", hue="group",
                alpha=0.6, s=40, edgecolor="none", palette=palette)
# mean 点
sns.scatterplot(data=df_tsne[df_tsne["is_mean"]],
                x="tsne_x", y="tsne_y", hue="group",
                s=220, markers=["X", "P"], style="group",
                legend=False, edgecolor="black", linewidth=1, palette=palette)
plt.title(f"t-SNE of activations ({target_file}, top/bottom 30% reasoning length)")
plt.tight_layout()
plt.show()
...
sns.histplot(
    data=data_information_df,
    x="avg_tokens_reasoning",
    hue="level",
    element="step",
    stat="density",
    common_norm=False,
...
sns.kdeplot(data=data_information_df, x="avg_tokens_reasoning", hue="level", common_norm=False, lw=1)
plt.xlabel("avg_tokens_reasoning")
plt.ylabel("Density")
plt.title("Distribution by level")
plt.tight_layout()
plt.show()

# 可选：箱线/小提琴图，便于对比中位数与分布形态
plt.figure(figsize=(10, 5))
sns.violinplot(data=data_information_df, x="level", y="avg_tokens_reasoning", inner="box", cut=0)
plt.xlabel("level")
plt.ylabel("avg_tokens_reasoning")
plt.title("Per-level distribution")
plt.tight_layout()
plt.show()
...
# 选择你要看的层（避免 36 就别用 36）
layer_id = 36
repr_files = sorted([f for f in os.listdir(representation_path)
                     if f.startswith("embeds_") and f.endswith(".npy")],
                    key=lambda x: int(os.path.splitext(x)[0].split("_")[-1]))
emb = np.load(os.path.join(representation_path, repr_files[layer_id]))

lengths = data_information_df["avg_tokens_reasoning"].to_numpy()
assert len(emb) == len(lengths)
low_thr, high_thr = np.quantile(lengths, [0.3, 0.7])
idx_short = np.where(lengths <= low_thr)[0]
idx_long = np.where(lengths >= high_thr)[0]
...
steering_vectors = []  # 按层存差值
for layer_id, fname in enumerate(repr_files):
    emb = np.load(os.path.join(representation_path, fname))
    # 若长度不一致则跳过/截断
    if emb.shape[0] < max(idx_short.max(), idx_long.max()) + 1:
        print(f"[skip] {fname} length mismatch")
        continue
    mean_short = emb[idx_short].mean(axis=0)
    mean_long = emb[idx_long].mean(axis=0)
    vec = mean_long - mean_short  # 长-短，方向自行约定
    steering_vectors.append(vec)

steering_vectors = np.stack(steering_vectors)  # shape [num_layers, hidden_dim]
np.save("steering_vectors_length_diff.npy", steering_vectors)
print("saved steering_vectors_length_diff.npy", steering_vectors.shape)


# %%
for i in range(len(mean_steering_vectors)):
    square = np.square(steering_vectors[i])
    sum_square = np.sum(square)
    norm = np.sqrt(sum_square) 
    print("Layer ", i, " norm: ", norm)

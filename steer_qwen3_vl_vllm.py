...
if os.environ.get("STEER_QWEN3_REGISTER") == "1":
    print(
        f"[steer] Imported steer_qwen3_vl_vllm (pid={os.getpid()}).",
        flush=True,
    )
...
def _steer_debug_log(message: str) -> None:
    print(message, file=sys.stderr, flush=True)


if _dynamo_disable is not None:
    _steer_debug_log = _dynamo_disable(_steer_debug_log)
...
class SteerQwen3DecoderLayer(Qwen3DecoderLayer):
    def __init__(
        self,
        config: Any,
        ...
        steering_strength: torch.Tensor | None = None,
        adaptive_steering: bool = False,
        adaptive_cfg: dict[str, float | str | None] | None = None,
    ) -> None:
        super().__init__(
            config=config,
        ...
        self.register_buffer("steering_strength", steering_strength)
        self.register_buffer("calibration_activation", None)
        self.register_buffer("adaptive_alpha", None)
        self.adaptive_steering = bool(adaptive_steering)
        self._set_adaptive_cfg(adaptive_cfg or {})

    def _set_adaptive_cfg(self, cfg: dict[str, float | str | None]) -> None:
        self.adaptive_cfg = dict(cfg)
        self.adaptive_reduce = str(cfg.get("reduce", "last")).lower()
        eps = cfg.get("eps", 1e-6)
        self.adaptive_eps = float(eps) if eps is not None else 1e-6

    def _summarize_hidden_states(self, hidden_states: torch.Tensor) -> torch.Tensor:
        reduce = self.adaptive_reduce
    ...
    def _adaptive_strength(self, hidden_states: torch.Tensor) -> torch.Tensor:
        summary = self._summarize_hidden_states(hidden_states)
        calib = self.calibration_activation
        if calib is not None:
            calib = calib.to(device=summary.device, dtype=summary.dtype)
            if summary.dim() == 3:
                summary = summary - calib.view(1, 1, -1)
            else:
                summary = summary - calib.view(1, -1)
        vec = self.steering_vector.to(device=summary.device, dtype=summary.dtype)
        alpha = self.adaptive_alpha
        if alpha is None:
            alpha = self.steering_strength
        alpha = alpha.to(device=summary.device, dtype=summary.dtype)
        summary_f = summary.float()
        vec_f = vec.float()
        norm_summary = torch.norm(summary_f, dim=-1) + self.adaptive_eps
        norm_vec = torch.norm(vec_f) + self.adaptive_eps
        cos = (summary_f * vec_f).sum(dim=-1) / (norm_summary * norm_vec)
        scale = F.relu(cos)
        strength = -alpha * (scale / norm_vec)
        return strength
    ...
    def forward(
        ...
        assert self.steering_strength == self.steering_strength  # Avoid heisenbug.
        """
        delta = self.steering_vector * self.steering_strength
        before_mean = hidden_states.mean().item()
        after_mean = (hidden_states + delta).mean().item()
        delta_norm = delta.norm().item()
        print(delta)
        if delta_norm == 0.0:
            raise RuntimeError("Steering delta is zero; activation is unchanged.")
        else:
            raise RuntimeError("Steering applied; activation is changed.")
        _steer_debug_log(
            f"[steer-debug] mean_before={before_mean:.6f} "
            f"mean_after={after_mean:.6f} delta_norm={delta_norm:.6f} "
            f"hidden_shape={tuple(hidden_states.shape)}"
        )
        """
        vec = self.steering_vector.to(device=hidden_states.device, dtype=hidden_states.dtype)
        if self.adaptive_steering:
            strength = self._adaptive_strength(hidden_states)
            strength = strength * self.steering_strength.to(
                device=hidden_states.device, dtype=hidden_states.dtype
            )
        else:
            strength = self.steering_strength.to(
                device=hidden_states.device, dtype=hidden_states.dtype
            )
        if os.environ.get("STEER_DEBUG_STRENGTH", "") in {"1", "true", "yes", "on"}:
            s = strength.detach().float().reshape(-1)
            _steer_debug_log(
                f"[steer-debug] strength mean={s.mean().item():.6f} "
                f"min={s.min().item():.6f} max={s.max().item():.6f} "
                f"shape={tuple(strength.shape)}"
            )

        if hidden_states.dim() == 3:
            if strength.dim() == 0 or strength.numel() == 1:
                delta = vec.view(1, 1, -1) * strength
            elif strength.dim() == 1:
                delta = vec.view(1, 1, -1) * strength.view(-1, 1, 1)
            else:
                delta = vec.view(1, 1, -1) * strength.unsqueeze(-1)
        elif hidden_states.dim() == 2:
            if strength.dim() == 0 or strength.numel() == 1:
                delta = vec * strength
            else:
                delta = vec.unsqueeze(0) * strength.view(-1, 1)
        else:
            delta = vec * strength
...
class Qwen3LLMModel(Qwen3Model):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        Qwen2Model.__init__(
            ...
            prefix=prefix,
            decoder_layer_type=SteerQwen3DecoderLayer,
        )
        ...
        steering_vector_path = os.environ.get("steering_vector_path")
        assert steering_vector_path is not None, "steering_vector_path is not set"
        steering_strength_str = os.environ.get("steering_strength_list")
        assert steering_strength_str is not None, "steering_strength is not set"

        adaptive_flag = os.environ.get("steering_adaptive", "")
        adaptive_steering = str(adaptive_flag).lower() in {"1", "true", "yes", "y", "on"}
        adaptive_cfg: dict[str, float | str | None] = {}
        adaptive_cfg_str = os.environ.get("steering_adaptive_config")
        if adaptive_cfg_str:
            try:
                parsed = json.loads(adaptive_cfg_str)
                if isinstance(parsed, dict):
                    adaptive_cfg = parsed
            except Exception:
                for part in adaptive_cfg_str.split(","):
                    if "=" not in part:
                        continue
                    ...
                    if not key:
                        continue
                    try:
                        adaptive_cfg[key] = float(value)
                    except ValueError:
                        adaptive_cfg[key] = value
        
        adaptive_alpha_str = os.environ.get("steering_adaptive_alpha")
        if adaptive_steering and adaptive_alpha_str is None:
            raise RuntimeError("steering_adaptive_alpha is not set")

        print("Loading steering vectors from:", steering_vector_path, flush=True)
        print("Steering strengths:", steering_strength_str, flush=True)
        if adaptive_steering:
            print("Adaptive alpha:", adaptive_alpha_str, flush=True)
        ...
        try:
            ...
            dtype = vllm_config.dtype if hasattr(vllm_config, "dtype") else torch.bfloat16

        steering_vectors = torch.from_numpy(np.load(steering_vector_path)).to(
            device=device, dtype=dtype
        )
        steering_strengths = torch.tensor(
            list(map(float, steering_strength_str.split(","))),
            device=device,
            dtype=dtype,
        )
        adaptive_alpha: torch.Tensor | None = None
        if adaptive_alpha_str is not None:
            adaptive_alpha = torch.tensor(
                float(adaptive_alpha_str),
                device=device,
                dtype=dtype,
            )
        calibration_path = os.environ.get("steering_calibration_path")
        calibration_vectors = None
        if calibration_path:
            print("Loading calibration vectors from:", calibration_path, flush=True)
            calibration_vectors = torch.from_numpy(np.load(calibration_path)).to(
                device=device, dtype=dtype
            )
        assert steering_vectors.device == device
        assert steering_strengths.device == device
        assert steering_vectors.dtype == dtype
        assert steering_strengths.dtype == dtype

        num_layers = int(self.config.num_hidden_layers)
        assert steering_vectors.shape[0] == num_layers, "steering vectors size mismatch"
        assert steering_strengths.numel() == num_layers, "steering strengths size mismatch"
        if adaptive_steering:
            assert adaptive_alpha is not None
        if calibration_vectors is not None:
            if calibration_vectors.shape[0] == num_layers + 1:
                print(
                    "[warn] calibration vectors include embedding layer; dropping index 0.",
                    flush=True,
                )
                calibration_vectors = calibration_vectors[1:]
            assert calibration_vectors.shape[0] == num_layers, "calibration vectors size mismatch"

        for layer_idx, layer in enumerate(self.layers):
            if isinstance(layer, SteerQwen3DecoderLayer):
                layer.steering_vector = steering_vectors[layer_idx]
                layer.steering_strength = steering_strengths[layer_idx]
                layer.adaptive_steering = adaptive_steering
                if adaptive_steering and adaptive_alpha is not None:
                    layer.adaptive_alpha = adaptive_alpha
                layer._set_adaptive_cfg(adaptive_cfg)
                if calibration_vectors is not None:
                    layer.calibration_activation = calibration_vectors[layer_idx]
...
class SteerQwen3VLForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "model"):
        super().__init__()
        print(
            f"[steer] Using SteerQwen3VLForConditionalGeneration (pid={os.getpid()}).",
            flush=True,
        )
